...
import { useCreateAndPay } from "../hooks/useCreateAndPay";
import { FaArrowLeft } from "react-icons/fa";
import Link from "next/link";
import Image from "next/image";

function CreateAndPayContent() {
  ...
  const { loading, errorMsg, successMsg, createAndPay } = useCreateAndPay();

  useEffect(() => {
    if (typeof window !== "undefined") {
      const storedGroupId = localStorage.getItem("createdGroupId");
  ...
  function validateForm() {
    ...
    if (!phoneNumber.trim()) {
      ...
      isValid = false;
    } else if (!/^(\+2547\d{8}|07\d{8}|7\d{8})$/.test(phoneNumber.trim())) {
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    if (!groupId) {
      alert("Group information is missing. Please create a group.");
      return;
    }
    ...
    let type = null;
    if (typeof window !== "undefined") {
      type = localStorage.getItem("selectedKirchaType");
    }

    let amountToPay = priceFull;
    if (type === "half") {
      amountToPay = priceFull / 20;
    } else if (type === "full") {
      amountToPay = priceFull / 10;
    }
    ...
    if (!/^2547\d{8}$/.test(formattedPhone)) {
      alert("Please enter a valid Kenyan phone number starting with +2547 or 07");
      return;
    }
  ...
  return (
    <div className="min-h-screen bg-[#4b0000] flex flex-col items-center px-4 pt-10">
      ...
      <form onSubmit={handleSubmit} className="w-200 max-w-full space-y-6 text-lg">
        <div>
          <label htmlFor="fullNameInput" className="block text-white font-bold mb-1">Full name</label>
          <input
            id="fullNameInput"
            type="text"
            value={fullName}
        ...
        <div>
          <label htmlFor="phoneInput" className="block text-white font-bold mb-1">Phone number</label>
          <input
            id="phoneInput"
            type="tel"
            value={phoneNumber}
            ...
            className="w-full rounded px-4 py-4 text-white text-lg border border-white outline-none"
            required
            pattern="(\+2547\d{8}|07\d{8}|7\d{8})"
            title="Valid Kenyan phone number starting with +2547 or 07"
          />
          {errors.phoneNumber && <p className="text-red-400">{errors.phoneNumber}</p>}
        </div>
        <div>
          <label htmlFor="addressInput" className="block text-white font-bold mb-1">Delivery Address</label>
          <input
            id="addressInput"
            type="text"
        ...
        {successMsg && <p className="text-white text-center font-semibold">{successMsg}</p>}
        <button
          type="submit"
          disabled={loading || !groupId || !priceFull}
          className="w-full bg-red-600 text-white py-4 rounded font-semibold hover:bg-red-700 transition text-xl mt-20"
        >
...
export default function CreateAndPayPageWrapper() {
  return (
